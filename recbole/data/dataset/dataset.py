...
class Dataset(torch.utils.data.Dataset):
    ...
    def _from_scratch(self):
        ...
        self._load_data(self.dataset_name, self.dataset_path)  #将数据从文件加载到内存，格式不变
        self._init_alias()
    ...
    def build(self):
        """
        根据指定的数据分割策略创建数据加载器，并且通过滑动窗口生成子交互序列生成训练数据
        ...
        """
        self._change_feat_format()   #这里就已经把数据进行滑动窗口处理了，这里有没有对数据进行分割
        ...
        elif split_mode == "LS":  #这里对交互序列集合进行按照group——by指定的列进行分组，一般是user——id，然后对每个用户的子序列集合进行LOO操作，比如一个用户有4个子序列，前两个为训练集，后两个分别是验证和测试集
