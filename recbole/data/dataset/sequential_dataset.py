...
class SequentialDataset(Dataset):
    ...
    def _change_feat_format(self):
        """
        将dataframe一行一个交互的形式转化为interaction一行一个用户交互序列的格式
        Change feat format from :class:`pandas.DataFrame` to :class:`Interaction`,
        then perform data augmentation.
        """
        super()._change_feat_format()  #将df转化为了interaction集合，但是没有处理，和文件内容一样
        print(self.inter_feat['timestamp'])
        ...
        for field in self.inter_feat:
            if field != self.uid_field:
                ...
                for i, (index, length) in enumerate(
                    ...
                    new_dict[list_field][i][:length] = value[index]  #就是把当前交互序列i的对应的下标二元组转化为tensor的item列表替换掉填充的0列表，比如最大50的tensor
