...
from recbole.trainer.multi_loss_former import GradNormLossForm
import numpy as np
import torch
...
from recbole.data import FastNegSampleEvalDataLoader
...
from recbole.utils.load_class_utils import load_class_from_file
...
class Trainer(AbstractTrainer):
    ...
    def __init__(self, config, model):
        ...
        self.multi_loss_form = config['mutil_loss_form']
        if self.multi_loss_form:
            #初始化损失函数的处理对象
            self.init_mutil_loss_form()
        #初始化损失函数的缩放比例
        self.scale_ratio = config['scale_ratio']

    def init_mutil_loss_form(self):
        class_name = self.multi_loss_form
        path_name = 'multi_loss_former.py'
        file_path = os.path.join(os.path.dirname(__file__), path_name)
        self.loss_form = load_class_from_file(file_path, class_name)(self.model, self.config)
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        ...
        for batch_idx, interaction in enumerate(iter_data):
            ...
            if isinstance(losses, tuple):
                """
                这里加一个多任务损失处理的类，初始化的时候从trainer初始化 
                """
                origin_loss_tuple = tuple(per_loss.item() for per_loss in losses)  #这是原始损失
                #这里是对没有权重和缩小尺度的损失进行尺度缩小
                losses = tuple([loss * self.scale_ratio for loss in losses])
                if self.loss_form:
                    loss, losses, is_backward = self.loss_form(losses)
                else:
                    loss = sum(losses)
                # loss_tuple = tuple(per_loss.item() for per_loss in origin_losses) 这是累积的加权后的损失
                total_loss = (
                    origin_loss_tuple
                    if total_loss is None
                    else tuple(map(sum, zip(total_loss, origin_loss_tuple)))
                )
            else:
                losses = losses * self.scale_ratio
                loss = losses
                total_loss = (
                    losses.item() if total_loss is None else total_loss + losses.item()
                )
            self._check_nan(loss)
            if is_backward is None or is_backward is False:
                scaler.scale(loss).backward()  #用于混合精度的代码，他将loss方法后再进行的反向传播
    ...
    def fit(
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                valid_end_time = time()
                #计算当前epoch的平均权重
                if hasattr(self.loss_form, "weight_records"):
                    epoch_steps = int(len(self.loss_form.weight_records[0]) / (epoch_idx + 1))
                    weights_str = ""
                    weights_list = self.loss_form.loss_weights.tolist()
                    for idx, weight in enumerate(weights_list):
                        avg_weight = sum(self.loss_form.weight_records[idx][-epoch_steps:]) / epoch_steps
                        avg_weight = str(round(avg_weight, 4))
                        weights_str += " " + set_color("weight" + str(idx + 1), color="blue") + ": " + str(
                            round(weight, 4)) + f" |{avg_weight}"
                    valid_score_output = (
                ...
                if stop_flag:
                    ...
                    if self.loss_form:
                        self.loss_form.save_weights_records()
                    break
    ...
    def _neg_sample_batch_eval(self, batched_data):
        ...
        if batch_size <= self.test_batch_size:
            origin_scores = self.model.predict(interaction.to(self.device))  #计算每个交互序列对其正样本的得分
        else:  #这里不需要
            origin_scores = self._spilt_predict(interaction, batch_size)

        if self.config["eval_type"] == EvaluatorType.VALUE:  #这里不需要
            ...
            batch_user_num = positive_u[-1] + 1  #当前batch有几个用户
            scores = torch.full(
                (batch_user_num, self.tot_item_num), -np.inf, device=self.device
            )  #创建一个用来记录每个用户得分的无穷小数张量
            scores[row_idx, col_idx] = origin_scores  #将样本得分标记进去
            return interaction, scores, positive_u, positive_i
    ...
    def evaluate(
        ...
        self.model.eval()  #将模型调整为评估阶段。不进行梯度传播和随即操作，比如dropout

        if isinstance(eval_data, FullSortEvalDataLoader):  #如果eval_mode: full
            eval_func = self._full_sort_batch_eval
            if self.item_tensor is None:
                self.item_tensor = eval_data._dataset.get_item_feature().to(self.device)
        elif isinstance(eval_data, FastNegSampleEvalDataLoader):
            eval_func = self._fast_neg_sample_batch_eval
        else:  #如果eval_mode: unixxx  再加一个新的判别
            eval_func = self._neg_sample_batch_eval  #设置当前的评价模式位负采样模式
        ...
        num_sample = 0
        for batch_idx, batched_data in enumerate(iter_data):  #这里调用一次batch的数据，就调用一次sample的call_fn
            num_sample += len(batched_data)
            #这里的batched_data是当前batch信息，其中positive_u表示batch包含几个用户，positive_i表示每个用户的正样本id
            #其中最特殊的是每个batch的interaction，他是根据评估mode决定的，比如采用uni100，那么他会为每个用户交互序列生成
            #100个目标预测项是负样本的交互序列，所以一个用户对应1个目标是正样本的和100个负样本的，共101个序列，假如十个用户
            #那么就有1010个交互序列，就是the batch of interaction: 1010
            #将正负交互序列当作一个batch一起送进推理，就可以一次获得所有正负样本的得分，从而进行指标的计算
